...
const ProjectCard: React.FC<ProjectCardProps> = ({ 
  ...
  return (
    <Card className={`glass overflow-hidden opacity-0 animate-fade-in ${rotateValue} hover:rotate-0 transition-all duration-300 relative`} style={{ animationDelay }}>
      {featured && (
        <div className="absolute top-4 right-4 z-30 bg-street-orange/90 text-white text-xs py-1 px-2 rounded-md flex items-center gap-1 animate-pulse">
          <Flame className="h-3 w-3" />
          <span>HOT</span>
        </div>
      )}
      
      <div className="absolute top-4 left-4 z-30 bg-black/60 backdrop-blur-sm text-white text-xs py-1 px-2 rounded-md flex items-center gap-1">
      ...
      <div className="relative aspect-video w-full overflow-hidden group">
        ...
        {/* Decorative elements */}
        <div className="absolute top-0 left-0 w-full h-full z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      <div className="p-6 relative">
        ...
        <Button asChild variant="ghost" className="text-street-purple hover:bg-street-purple/10 flex items-center gap-2 group relative">
...
const ProjectsSection: React.FC = () => {
  const projects = [
    ...
    {
      title: "Sneakers Collector",
      description: "App per collezionisti di sneakers con marketplace, autenticazione e valutazione automatica.",
      imageSrc: "https://images.unsplash.com/photo-1552346154-21d32810aba3",
      technologies: ["React Native", "GraphQL", "AWS", "Tensorflow", "Stripe"],
      category: "Mobile",
      link: "#"
    },
  ];
  ...
  return (
    <section id="projects" className="py-20 relative">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute center top-1/2 left-1/2 w-96 h-96 -translate-x-1/2 -translate-y-1/2 rounded-full bg-street-neon/5 filter blur-3xl"></div>
        ...
        <div className="absolute top-20 left-20 w-32 h-32 border-2 border-dashed border-street-orange/20 rounded-full animate-spin-slow"></div>
        <div className="absolute bottom-40 right-40 w-24 h-24 border border-street-neon/30 rotate-45 animate-pulse-slow"></div>
      </div>
      ...
      <div className="container">
        <div className="text-center mb-16 opacity-0 animate-fade-in">
          ...
          <p className="text-gray-400 md:text-lg max-w-2xl mx-auto">
            Soluzioni creative che abbiamo sviluppato per clienti 
            <span className="text-street-orange"> street-smart</span>.
            Tecnologie all'avanguardia con design che 
            <span className="text-street-neon"> spacca</span>.
          </p>
        </div>
        ...
        <div className="mt-12 text-center opacity-0 animate-fade-in" style={{ animationDelay: "0.4s" }}>
          <Button size="lg" className="bg-street-purple hover:bg-street-purple/80 text-white transform rotate-1 hover:rotate-0 hover:scale-105 transition-all group">
            <span className="relative z-10">Visualizza Tutti i Progetti</span>
            <span className="absolute inset-0 bg-gradient-to-r from-street-purple to-street-pink opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-md"></span>
          </Button>
        </div>
